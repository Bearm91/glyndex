package com.bearm.glyndex.helpers;

public class Constants {

    public static final String CATEGORY_ID_FIELD = "CategoryId";
    public static final String CATEGORY_NAME_FIELD = "CategoryName";
    public static final String SEARCH_BOOLEAN = "IsSearch";
    public static final String FOOD_ID_FIELD = "FoodId";
    public static final String FOOD_FORM_MODE = "foodFormMode";

    public static final Integer RESULT_CODE_OK = 1;
    public static final Integer REQUEST_CODE_OK = 1;

    public static final String SHARED_PREFERENCES_ONBOARDING_KEY = "onBoarding";
    public static final String SHARED_PREFERENCES_ONBOARDING_SHOWINFO = "showInfo";
    public static final String SHARED_PREFERENCES_V23_SHOWINFO = "showInfoV23";

    public static final Integer GRAMS_IN_CHRATION = 10;

    public static final String FOOD_FORM_EDIT_MODE = "foodEditMode";
    public static final String FOOD_FORM_CREATE_MODE = "foodCreateMode";
}
